(function () {
  ...
  const youtubeIcon = `<svg viewBox="0 0 576 512"><path fill="currentColor" d="M549.7 124.1c-6.3-23.6-24.8-41.5-48.3-47.8C458.8 64.8 288 64.8 288 64.8s-170.8 0-213.4 11.5c-23.5 6.3-42 24.2-48.3 47.8C14.8 166.7 14.8 256 14.8 256s0 89.3 11.5 131.9c-6.3 23.6 24.8 41.5 48.3 47.8C117.2 447.2 288 447.2 288 447.2s170.8 0 213.4-11.5c23.5-6.3 42-24.2 48.3-47.8C561.2 345.3 561.2 256 561.2 256s0-89.3-11.5-131.9zM232 338.1V173.9L361 256 232 338.1z"/></svg>`;
  const fullscreenIcon = `<svg viewBox="0 0 640 640"><path fill="currentColor" d="M128 96C110.3 96 96 110.3 96 128L96 224C96 241.7 110.3 256 128 256C145.7 256 160 241.7 160 224L160 160L224 160C241.7 160 256 145.7 256 128C256 110.3 241.7 96 224 96L128 96zM416 96C398.3 96 384 110.3 384 128C384 145.7 398.3 160 416 160L480 160L480 224C480 241.7 494.3 256 512 256C529.7 256 544 241.7 544 224L544 128C544 110.3 529.7 96 512 96L416 96zM128 384C110.3 384 96 398.3 96 416L96 512C96 529.7 110.3 544 128 544L224 544C241.7 544 256 529.7 256 512C256 494.3 241.7 480 224 480L160 480L160 416C160 398.3 145.7 384 128 384zM512 384C494.3 384 480 398.3 480 416L480 480L416 480C398.3 480 384 494.3 384 512C384 529.7 398.3 544 416 544L512 544C529.7 544 544 529.7 544 512L544 416C544 398.3 529.7 384 512 384z"/></svg>`;
  ...
  window.BloxcraftIdleTab = {
    ...
    setPreset(name) {
      if (!IDLE_PRESETS[name] || name === "none") return false;
      saveIdleConfig({ ...IDLE_PRESETS[name] });
      if (document.hidden) applyIdleTab();
      else applyOriginalTab();
      return true;
    },
    async setCustom(title, favicon) {
      title = String(title || "").trim();
      favicon = String(favicon || "").trim();
      if (!title || !favicon) throw new Error("Missing title or favicon URL");

      await new Promise((resolve, reject) => {
        const img = new Image();
        img.onload = () => resolve();
        img.onerror = () => reject(new Error("Failed to load favicon URL"));
        img.src = favicon;
      });
    ...
    presets: {
      none: { ...IDLE_PRESETS.none },
      classroom: { ...IDLE_PRESETS.classroom },
      docs: { ...IDLE_PRESETS.docs },
      drive: { ...IDLE_PRESETS.drive }
    }
  };
})();
