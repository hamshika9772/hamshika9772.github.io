...
const IDLE_PRESETS = {
  ...
  classroom: {
    ...
    favicon: "/idle-favicons/clasroom.ico"
  },
  docs: {
    type: "preset",
    preset: "docs",
...
const originalFaviconHref = (() => {
  ...
  return existing ? existing.href : "/favicon.ico";
})();
...
function loadIdleConfig() {
  try {
    ...
    if (!parsed || typeof parsed !== "object") return { ...IDLE_PRESETS.none };

    if (parsed.type === "none") return { ...IDLE_PRESETS.none };

    if (parsed.type === "preset" && parsed.preset && IDLE_PRESETS[parsed.preset]) {
      return { ...IDLE_PRESETS[parsed.preset] };
    }
...
function saveIdleConfig(config) {
  idleConfig = config;
  localStorage.setItem(IDLE_STORAGE_KEY, JSON.stringify(config));
}
...
function applyIdleTab() {
  ...
  if (idleConfig.type === "preset") {
    const preset = IDLE_PRESETS[idleConfig.preset];
    if (!preset) {
      applyOriginalTab();
      return;
    }
    document.title = preset.title;
    getFaviconEl().href = preset.favicon;
    return;
  }

  if (idleConfig.type === "custom") {
    if (!idleConfig.title || !idleConfig.favicon) {
      applyOriginalTab();
      return;
    }
    document.title = idleConfig.title;
    getFaviconEl().href = idleConfig.favicon;
    return;
  }
...
function getCurrentWispMode() {
  const saved = localStorage.getItem(WISP_STORAGE_KEY) || "wss://wisp.classroom.lat/";
  if (saved === "wss://wisp.classroom.lat/") {
    return {
      mode: "default",
      value: "wss://wisp.classroom.lat/"
    };
  }
  return {
    mode: "custom",
...
function renderIdleSummary() {
  if (!idleConfig || idleConfig.type === "none") return "None";
  if (idleConfig.type === "preset") {
    const preset = IDLE_PRESETS[idleConfig.preset];
    return preset ? preset.label : "None";
  }
  if (idleConfig.type === "custom") return `Custom: ${idleConfig.title || "Untitled"}`;
  return "None";
}
...
function renderSettingsPanel() {
  ...
  const wisp = getCurrentWispMode();
  const idleSummary = renderIdleSummary();

  body.innerHTML = `
    <div class="settings-section">
      <div class="settings-section-title">Idle Tab</div>
      <div class="settings-row">
        <label>Preset</label>
        <select id="idlePresetSelect">
      ...
      <div id="idleCustomFields" class="settings-subgroup" style="display:none">
        <div class="settings-row">
          <label>Custom Title</label>
          <input id="idleCustomTitle" type="text" placeholder="Google Docs">
        </div>
        <div class="settings-row">
          <label>Custom Favicon URL</label>
          <input id="idleCustomFavicon" type="text" placeholder="https://example.com/icon.ico">
        </div>
      </div>

      <div class="settings-actions">
        <button id="saveIdleBtn">Save Idle Tab</button>
      </div>

      <div class="settings-note">Current: ${idleSummary}</div>
    </div>
    ...
    <div class="settings-section">
      ...
      <div class="settings-row">
        <label>Mode</label>
        <select id="wispModeSelect">
          <option value="default">wisp.classroom.lat</option>
          <option value="custom">Custom</option>
        </select>
      </div>

      <div id="wispCustomWrap" class="settings-subgroup" style="display:none">
        <div class="settings-row">
          <label>Custom Wisp URL</label>
          <input id="wispCustomInput" type="text" placeholder="wss://example.com/">
        </div>
      </div>

      <div class="settings-actions">
        <button id="saveWispBtn">Save Wisp Server</button>
      </div>

      <div class="settings-note">Current: ${wisp.value}</div>
    </div>

    <div class="settings-section">
      <div class="settings-section-title">Extra Navbar Buttons</div>
      <div class="settings-actions">
        <button id="openExtraNavManagerBtn">Manage Extra Navbar Buttons</button>
      </div>
      <div class="settings-note">${extraNavs.length} button${extraNavs.length === 1 ? "" : "s"} enabled</div>
    </div>
  `;

  const idlePresetSelect = document.getElementById("idlePresetSelect");
  const idleCustomFields = document.getElementById("idleCustomFields");
  const idleCustomTitle = document.getElementById("idleCustomTitle");
  const idleCustomFavicon = document.getElementById("idleCustomFavicon");

  if (idleConfig.type === "preset") {
    idlePresetSelect.value = idleConfig.preset;
  } else if (idleConfig.type === "custom") {
    idlePresetSelect.value = "custom";
    idleCustomFields.style.display = "block";
    idleCustomTitle.value = idleConfig.title || "";
    idleCustomFavicon.value = idleConfig.favicon || "";
  } else {
    idlePresetSelect.value = "none";
  }

  idlePresetSelect.addEventListener("change", () => {
    idleCustomFields.style.display = idlePresetSelect.value === "custom" ? "block" : "none";
  });

  document.getElementById("saveIdleBtn").addEventListener("click", async () => {
    const value = idlePresetSelect.value;

    if (value === "none") {
      saveIdleConfig({ ...IDLE_PRESETS.none });
      if (document.hidden) applyIdleTab();
      else applyOriginalTab();
      renderSettingsPanel();
      alert("Idle tab saved.");
      return;
    }

    if (value === "custom") {
      const title = idleCustomTitle.value.trim();
      const favicon = idleCustomFavicon.value.trim();

      if (!title || !favicon) {
        alert("Enter both a custom title and favicon URL.");
        return;
      }

      try {
        await validateFaviconUrl(favicon);
        saveIdleConfig({
          type: "custom",
          label: "Custom",
          title,
        ...
        renderSettingsPanel();
        alert("Custom idle tab saved.");
      } catch {
        alert("Could not load that favicon URL.");
      }
      return;
    }

    if (!IDLE_PRESETS[value]) {
      alert("Invalid idle tab preset.");
      return;
    }

    saveIdleConfig({ ...IDLE_PRESETS[value] });
    if (document.hidden) applyIdleTab();
    else applyOriginalTab();
    renderSettingsPanel();
    alert("Idle tab saved.");
  ...
  const wispModeSelect = document.getElementById("wispModeSelect");
  const wispCustomWrap = document.getElementById("wispCustomWrap");
  const wispCustomInput = document.getElementById("wispCustomInput");

  wispModeSelect.value = wisp.mode;
  if (wisp.mode === "custom") {
    wispCustomWrap.style.display = "block";
    wispCustomInput.value = wisp.value;
  }

  wispModeSelect.addEventListener("change", () => {
    wispCustomWrap.style.display = wispModeSelect.value === "custom" ? "block" : "none";
  });

  document.getElementById("saveWispBtn").addEventListener("click", () => {
    if (wispModeSelect.value === "default") {
      localStorage.setItem(WISP_STORAGE_KEY, "wss://wisp.classroom.lat/");
      renderSettingsPanel();
      alert("Wisp server saved.");
      return;
    }

    const custom = wispCustomInput.value.trim();
    if (!custom) {
      alert("Enter a custom wisp URL.");
      return;
    }

    localStorage.setItem(WISP_STORAGE_KEY, custom);
    renderSettingsPanel();
    alert("Wisp server saved.");
  });

  document.getElementById("openExtraNavManagerBtn").addEventListener("click", () => {
...
function validateFaviconUrl(url) {
  return new Promise((resolve, reject) => {
    ...
    img.onerror = () => reject(new Error("Invalid favicon URL"));
    img.src = url;
  });
}
