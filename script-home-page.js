...
const TAB_CLOAK_KEY = "BloxcraftSecretUBGFaviconInjection";
const EXTRA_NAVS_KEY = "extraNavs";
const WISP_KEY = "proxy-ws";

const TAB_PRESETS = {
  docs: {
    icon: "/idle-favicons/docs.ico",
    title: "Google Docs"
  },
  drive: {
    icon: "/idle-favicons/drive.png",
    title: "Home - Google Drive"
  },
  classroom: {
    icon: "/idle-favicons/clasroom.ico",
...
let extraNavs = JSON.parse(localStorage.getItem(EXTRA_NAVS_KEY) || "[]");
let wispUrl = localStorage.getItem(WISP_KEY) || "wss://wisp.classroom.lat/";

function getCurrentFaviconEl() {
  let icon = document.querySelector("link[rel='icon']") || document.querySelector("link[rel='shortcut icon']") || document.querySelector("link[rel*='icon']");
  if (!icon) {
...
function normalizeTabConfig(raw) {
  if (!raw || typeof raw !== "object") {
    return {
      mode: "preset",
      preset: "classroom",
      title: TAB_PRESETS.classroom.title,
      icon: TAB_PRESETS.classroom.icon
    };
  }
  ...
  if (raw.mode === "custom") {
    return {
      mode: "custom",
      preset: "custom",
      title: raw.title || TAB_PRESETS.classroom.title,
      icon: raw.icon || TAB_PRESETS.classroom.icon
    };
  }

  const preset = TAB_PRESETS[raw.preset] ? raw.preset : "classroom";
  return {
    mode: "preset",
    preset,
    title: TAB_PRESETS[preset].title,
    icon: TAB_PRESETS[preset].icon
  };
}

function getSavedTabConfig() {
  try {
    const raw = JSON.parse(localStorage.getItem(TAB_CLOAK_KEY) || "null");
    return normalizeTabConfig(raw);
  } catch {
    return normalizeTabConfig(null);
  }
}

function saveTabConfig(config) {
  localStorage.setItem(TAB_CLOAK_KEY, JSON.stringify(config));
}

function applyTabCloak(config) {
  const finalConfig = normalizeTabConfig(config);
  document.title = finalConfig.title;
  getCurrentFaviconEl().href = finalConfig.icon;
}

function initTabCloak() {
  if (!localStorage.getItem(TAB_CLOAK_KEY)) {
    saveTabConfig({
      mode: "preset",
      preset: "classroom",
      title: TAB_PRESETS.classroom.title,
  ...
  applyTabCloak(getSavedTabConfig());
}
...
function toggleExtraOverlay() {
  const opening = extraOverlay.style.display !== "flex";
  extraOverlay.style.display = opening ? "flex" : "none";
  if (opening) settingsOverlay.style.display = "none";
}

function toggleSettingsOverlay() {
  const opening = settingsOverlay.style.display !== "block";
  settingsOverlay.style.display = opening ? "block" : "none";
  if (opening) {
...
function openExtraNavbarManager() {
...
function renderExtraOverlay() {
  ...
  predefinedExtras.forEach(item => {
    ...
    if (!exists) {
      ...
      addBtn.onclick = () => {
        extraNavs.push(item);
        localStorage.setItem(EXTRA_NAVS_KEY, JSON.stringify(extraNavs));
        renderExtraNavGroup();
        renderExtraOverlay();
      };
...
function syncSettingsUI() {
  const tabConfig = getSavedTabConfig();
  const tabPresetSelect = document.getElementById("tabPresetSelect");
  const customTabFields = document.getElementById("customTabFields");
  const customTabTitle = document.getElementById("customTabTitle");
  const customTabIcon = document.getElementById("customTabIcon");

  if (tabConfig.mode === "custom") {
    tabPresetSelect.value = "custom";
    customTabFields.style.display = "block";
    customTabTitle.value = tabConfig.title || "";
    customTabIcon.value = tabConfig.icon || "";
  } else {
    tabPresetSelect.value = tabConfig.preset || "classroom";
    customTabFields.style.display = "none";
    customTabTitle.value = "";
    customTabIcon.value = "";
  }

  const wispModeSelect = document.getElementById("wispModeSelect");
  const customWispWrap = document.getElementById("customWispWrap");
  const customWispInput = document.getElementById("customWispInput");
  const currentWisp = localStorage.getItem(WISP_KEY) || "wss://wisp.classroom.lat/";

  if (currentWisp === "wss://wisp.classroom.lat/") {
    wispModeSelect.value = "default";
    customWispWrap.style.display = "none";
    customWispInput.value = "";
  } else {
    wispModeSelect.value = "custom";
    customWispWrap.style.display = "block";
    customWispInput.value = currentWisp;
  }
}

function saveTabCloakSettings() {
  const preset = document.getElementById("tabPresetSelect").value;

  if (preset === "custom") {
    const title = document.getElementById("customTabTitle").value.trim() || TAB_PRESETS.classroom.title;
    const icon = document.getElementById("customTabIcon").value.trim() || TAB_PRESETS.classroom.icon;
    ...
    applyTabCloak(config);
    return;
  }

  const presetData = TAB_PRESETS[preset] || TAB_PRESETS.classroom;
  const config = {
    mode: "preset",
    preset,
...
function saveWispSettings() {
  const mode = document.getElementById("wispModeSelect").value;

  if (mode === "default") {
    wispUrl = "wss://wisp.classroom.lat/";
    localStorage.setItem(WISP_KEY, wispUrl);
    return;
  }

  let custom = document.getElementById("customWispInput").value.trim();
  if (!custom) custom = "wss://wisp.classroom.lat/";
  wispUrl = custom;
  localStorage.setItem(WISP_KEY, wispUrl);
}
...
function bindSettingsEvents() {
  ...
  tabPresetSelect.addEventListener("change", () => {
    customTabFields.style.display = tabPresetSelect.value === "custom" ? "block" : "none";
  });

  wispModeSelect.addEventListener("change", () => {
    customWispWrap.style.display = wispModeSelect.value === "custom" ? "block" : "none";
  });
}
...
function importSiteSave(input) {
  ...
  reader.onload = () => {
    try {
      ...
      if (typeof initTabCloak === "function") initTabCloak();
