          ...
          try {
            ...
            DATA.lupine = safeArray(d).map(g => {
              ...
              return normalize({
                ...
                altImg: "https://cdn.jsdelivr.gh/tharun9772/LupineVault@main/assets/images/games/large/" + encodedName + ".png",
        ...
        async function loadTGLSC() {
          ...
          try {
            ...
            DATA.tglsc = safeArray(d.data).map(g => {
              ...
              const img = baseImg.replace(/\/+$/, "") + "/" + (g.thumbnail || "").replace(/^\/+/, "");
              const embed = "/sail/embed/#" + (g.embed_url || "").replace(/^https?:\/\//, "");
              return {
                name: g.title,
                img,
                url: embed,
        ...
        async function loadSelenite() {
          ...
          try {
            ...
            DATA.selenite = safeArray(d).map(g => {
              ...
              const img = "https://math-quests-cc.dk-ubg.workers.dev/resources/semag/" + dir + "/" + g.image;
              const url = "/sail/embed/#https://selenite.cc/resources/semag/" + dir + "/index.html";
              return { name: g.name, img, url };
            }).filter(Boolean);
          } catch (e) {}
        }
        ...
        async function loadVelera() {
          ...
          try {
            ...
            DATA.velera = safeArray(d).map(g => {
              ...
              const cleanImg = String(g.image || "").replace(/^\/+/, "");
              const cleanLoc = String(g.location).replace(/^\/+/, "");
              return {
                name: g.title,
                img: "https://math-of-cc.dk-ubg.workers.dev/" + cleanImg,
                url: "/sail/embed/#https://velara.cc/" + cleanLoc
              };
            }).filter(Boolean);
          } catch (e) {}
        }
          ...
          el.onclick = async () => {
            ...
            if (cat === "all") {
              ...
              const loaderMap = {
                ...
                "3kh0lite": load3kh0Lite, tglsc: loadTGLSC, selenite: loadSelenite, velera: loadVelera
              };
